core_abundance_PAD4 <- function(seq_list, unique_core_list,ag_data){
    ...
    colnames(abundances) <- c("Core","Native","Autocit")
    ...
    for (i in 1:length_of_cores){ # For each core...
        ...
        if (num_peptides !=0){
            for (j in 1:num_peptides){
                peptide = core_index[[i]][[j]] # This should give us 143 for the first pass
                abundance = sum(c(ag_data$`Citrullinated_Replicate_1`[[peptide]],ag_data$`Citrullinated_Replicate_2`[[peptide]], ag_data$`Citrullinated_Replicate_3`[[peptide]], ag_data$`Citrullinated_Replicate_4`[[peptide]]),na.rm = TRUE)/num_peptides
